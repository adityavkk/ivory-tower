...
# ---------------------------------------------------------------------------
# Live tests -- adversarial with direct LLM executor
# ---------------------------------------------------------------------------

DIRECT_MODEL = "openai/claude-haiku-4-5"
DIRECT_API_BASE = "http://127.0.0.1:8112/v1"
DIRECT_TOPIC = (
    "Compare Python vs Go for building high-throughput HTTP microservices. "
    "Cover: performance characteristics, developer productivity, ecosystem "
    "maturity, and deployment trade-offs."
)
...
def _proxy_available() -> bool:
    """Check if the wibey proxy is reachable."""
    import urllib.request

    try:
        urllib.request.urlopen(f"{DIRECT_API_BASE}/models", timeout=3)
        return True
    except Exception:
        return False
...
skip_no_proxy = pytest.mark.skipif(
    not _proxy_available(), reason=f"proxy not reachable at {DIRECT_API_BASE}"
)
...
@skip_no_litellm
@skip_no_proxy
class TestDirectAdversarialLiveE2E:
    """Adversarial pipeline using the direct LLM executor (no counselors).

    Uses the wibey proxy at localhost:8112 with claude-haiku-4-5 for speed.
    max_rounds=1 to keep the test under ~5 minutes.
    """
    ...
    @pytest.fixture(autouse=True)
    def run_dir(self) -> Path:
        if TestDirectAdversarialLiveE2E._run_dir is None:
            config = RunConfig(
                topic=DIRECT_TOPIC,
                agents=["direct-a", "direct-b"],
                synthesizer="direct-synth",
                output_dir=RESEARCH_DIR,
                strategy="adversarial",
                max_rounds=1,
                verbose=True,
            ...
            TestDirectAdversarialLiveE2E._run_dir = run_pipeline(config)
        return TestDirectAdversarialLiveE2E._run_dir
    ...
    def test_manifest_valid(self, run_dir: Path):
        text = _assert_file_nonempty(run_dir / "manifest.json", "manifest.json")
        data = json.loads(text)
        assert data["strategy"] == "adversarial"
    ...
    def test_seed_reports_exist(self, run_dir: Path):
        for agent in ("direct-a", "direct-b"):
            _assert_file_nonempty(
                run_dir / "phase1" / f"{agent}-seed.md",
                f"{agent} seed report",
            )
    ...
    def test_optimization_scores_nonzero(self, run_dir: Path):
        """Direct mode should achieve 100% JSON parse success (no 0.0 scores)."""
        for agent in ("direct-a", "direct-b"):
            path = run_dir / "phase2" / f"{agent}-optimization-log.json"
            log = json.loads(path.read_text())
            assert log["seed_score"] > 0.0, (
                f"{agent} seed_score is 0.0 -- evaluation JSON parse likely failed"
            )

    def test_judging_round_debug_has_full_feedback(self, run_dir: Path):
        """round-debug.json in judging dirs should contain strengths/weaknesses."""
        phase2 = run_dir / "phase2"
        judging_dirs = sorted(
            d for d in phase2.glob("judging/round-*") if d.is_dir()
        )
        assert len(judging_dirs) >= 2, (
            f"Expected at least 2 judging dirs, found {len(judging_dirs)}"
        )
        for d in judging_dirs:
            debug_file = d / "round-debug.json"
            assert debug_file.exists(), f"Missing round-debug.json in {d.name}"
            data = json.loads(debug_file.read_text())
            if data.get("parse_success", False):
                assert len(data.get("strengths", [])) > 0, (
                    f"{d.name}: parse succeeded but no strengths in debug"
                )
                assert len(data.get("weaknesses", [])) > 0, (
                    f"{d.name}: parse succeeded but no weaknesses in debug"
                )

    def test_dimension_history_populated(self, run_dir: Path):
        m = Manifest.load(run_dir / "manifest.json")
        opt_phase = m.phases["adversarial_optimization"]
        for agent in ("direct-a", "direct-b"):
            seed = opt_phase.seeds[agent]
            assert len(seed.dimension_history) >= 1, (
                f"{agent} dimension_history is empty"
            )
            for entry in seed.dimension_history:
                assert "round" in entry
                assert "score" in entry
                assert entry["score"] > 0.0, (
                    f"{agent} round {entry['round']} scored 0.0"
                )
    ...
    def test_final_report_on_topic(self, run_dir: Path):
        ...
        assert any(
            term in text
            for term in ("python", "go", "microservice", "http", "throughput")
        ), "Final report doesn't appear to be on topic"
